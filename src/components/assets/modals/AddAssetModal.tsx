...
export default function AddAssetModal({
  ...
  const handleQuantityChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    // Only update formData if the input is a valid number
    const numericValue = parseFloat(inputValue);
    if (!isNaN(numericValue) && numericValue >= 0) {
      handleInputChange('quantity', numericValue);
      
      // Auto-convert between average cost and cost basis when quantity changes
      if (showAverageCost && formData.avgCost > 0) {
        // Convert average cost to cost basis
  ...
  const handleCostBasisChange = (value: number, currency: string) => {
    ...
    // Convert cost basis to average cost if quantity is available
  ...
  const handleAverageCostChange = (value: number, currency: string) => {
    ...
    // Convert average cost to cost basis if quantity is available
    if (formData.quantity > 0 && value > 0) {
  ...
  const handlePopularStockClick = async (stock: { symbol: string; name: string; exchange: string }) => {
    try {
      ...
      if (stockWithPrice) {
        // Create a merged stock object with the popular stock data
        const mergedStock = {
          ...stockWithPrice,
          symbol: stock.symbol,
          name: stock.name,
          exchange: stock.exchange,
          // Keep the original search result data for fields that might be missing from price data
  ...
  const handleStockSelect = async (stock: StockSearchResult) => {
    try {
      ...
      const stockWithPrice = await getStockWithPrice(stock.symbol);
      if (stockWithPrice) {
        // Merge the price data with the original search result to preserve country, sector, etc.
        const mergedStock = {
          ...stock,
          ...stockWithPrice,
          // Keep the original search result data for fields that might be missing from price data
      ...
      // Clear the search query to allow user to try again
      setSearchQuery('');
  ...
  const validateForm = () => {
    ...
    // Only validate the currently visible cost input
    if (showAverageCost && formData.avgCost < 0) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const assetData: CreateAssetInput = {
        ...
        currency: selectedStock.native_currency, // Store the native currency
